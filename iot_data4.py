...
import threading
from adDetection import ADDection
    ...
    def update_data(self):
        ...
        if number_of_update != 0:
            ...
            for data in update_data:
                ...
                abnormal_results = [MonitorName] if self.detect.S1(data = value, name = MonitorName) else []

                myquery = {"Timestamp": timestamp}
                findTimeStamp = self.mycol_to.find_one (myquery)

                if findTimeStamp:
                    isNormal = findTimeStamp['isNormal'] + abnormal_results
                    newvalues = {"$set": {MonitorName: value, 'isNormal': isNormal}}
                    finished = self.mycol_to.update_one (myquery, newvalues)
                    n +=1
                    # print ('成功修改 1 条数据')

                else:
                    isNormal = abnormal_results
